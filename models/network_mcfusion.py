...
class Cross_WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv_fx = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.qkv_fy = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
    ...
    def forward(self, x, y, mask=None):
        ...
        # 增强注意力机制
        B_, N, C = x.shape
        qkv_x = self.qkv_fx(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_y = self.qkv_fy(y).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv_x[0], qkv_x[1], qkv_x[2]  # make torchscript happy (cannot use tensor as tuple)
        q_y, k_y, v_y = qkv_y[0], qkv_y[1], qkv_y[2]

        q = q * self.scale
        attn = ((q @ k.transpose(-2, -1)) + (q_y @ k_y.transpose(-2, -1))) / 2

        # ######### old window attention
        # B_, N, C = x.shape
        # qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        #
...
class MCFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        # 2023.3.31更改原先的输入输出通道数真正转化为类VIF的卷积输入(两层)
        self.conv_first1_A = nn.Conv2d(in_chans, int(embed_dim/3), 3, 1, 1)
        self.conv_first1_B = nn.Conv2d(in_chans, int(embed_dim/3), 3, 1, 1)
        self.conv_first2_A = nn.Conv2d(int(embed_dim/3), int(embed_dim/3*2), 3, 1, 1)
        self.conv_first2_B = nn.Conv2d(int(embed_dim/3), int(embed_dim/3*2), 3, 1, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

        # 2023.4.5改 增加深度卷积支路  4.6改conv_AB从全连接变为核为3的decoder层 4.8改深度特征提取层的输出通道数扩大两倍
        self.conv_first3_A = nn.Conv2d(embed_dim, embed_dim*2, 3, 1, 1)
        self.conv_first3_B = nn.Conv2d(embed_dim, embed_dim*2, 3, 1, 1)
        self.conv_AB = nn.Conv2d(embed_dim*4, embed_dim*2, 1, 1, 0)
        self.conv_AB_1 = nn.Conv2d(embed_dim * 2, embed_dim , 3, 1, 1)
        ...
        # 5.19改动，增加双支路的重构RSTB
        self.layers_Re_A = nn.ModuleList()
        for i_layer in range(self.Re_num_layers):
            ...
            self.layers_Re_A.append(layer)
        self.norm_Re_A = norm_layer(self.num_features)

        self.layers_Re_B = nn.ModuleList()
        for i_layer in range(self.Re_num_layers):
            layer = RSTB(dim=embed_dim,
                         input_resolution=(patches_resolution[0],
                                           patches_resolution[1]),
            ...
            self.layers_Re_B.append(layer)
        self.norm_Re_B = norm_layer(self.num_features)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # 2023.3.29增加一层卷积保持前后卷积数量一致
            # 2023.4.5 将该层卷积变成120*60的
            self.conv_last0 = nn.Conv2d(embed_dim*2, embed_dim, 3, 1, 1)
    ...
    def forward_features_Ex_A(self, x):
        # Shallow Feature Extraction 为两层卷积 5.20 浅层特征提取部分
        # 2023.3.28 改为三层卷积与残差连接
        x1 = self.lrelu(self.conv_first1_A(x))   # 6*20*128*128
        x2 = self.lrelu(self.conv_first2_A(x1))   # 6*40*128*128
        x = self.lrelu(torch.cat([x1,x2],dim=1))

        # 2023.4.5改 增加深度卷积支路 局部深度特征提取部分
        x3 = self.lrelu(self.conv_first3_A(x))  # 60*60
    ...
    def forward_features_Ex_B(self, x):
        ...
        # 2023.3.28 改为三层卷积与残差连接
        x1 = self.lrelu(self.conv_first1_B(x))  # 6*20*128*128
        x2 = self.lrelu(self.conv_first2_B(x1))  # 6*40*128*128
        x = self.lrelu(torch.cat([x1, x2], dim=1))  # 20+40-- 6*60*128*128

        # 2023.4.5改 增加深度卷积支路
        x3 = self.lrelu(self.conv_first3_B(x))  # 60*60
    ...
    def forward_features_Fusion(self, x, y):
        ...
        y = self.norm_Ex_B(y)  # B L C



        # # CRSTB的layers_Fusion层
        ...
        #  RSTB
        for layer in self.layers_Re_A:
            x = layer(x, x_size)

        for layer in self.layers_Re_B:
            y = layer(y, y_size)
        x = self.norm_Re_A(x)  # B L C
        y = self.norm_Re_B(y)  # B L C
    ...
    def forward_deep_features_Fusion(self,x,x3,y3):
        x3 = self.lrelu(torch.cat([x3, y3], dim=1))
        x3 = self.lrelu(self.conv_AB(x3))
        x3 = self.lrelu(self.conv_AB_1(x3))
        x = self.lrelu(torch.cat([x, x3], dim=1))
        x = self.lrelu(self.conv_last0(x))
        return x
    ...
    def forward_features_Re(self, x):
        ...
        x = self.lrelu(self.conv_last3(x))
        return x
    ...
    def forward(self, A, B):
        ...
        # Feedforward 特征提取、特征融合、特征重构
        x, x3 = self.forward_features_Ex_A(x)   # 构成： shallow feature extraction（两个卷积）
        y, y3 = self.forward_features_Ex_B(y)
        x = self.forward_features_Fusion(x, y)  # 6*60*128*128
        x = self.forward_deep_features_Fusion(x, x3,y3)
...
if __name__ == '__main__':
    ...
    model = MCFusion(upscale=2, img_size=(height, width),
