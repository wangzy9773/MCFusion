...
class ModelPlain(ModelBase):
    ...
    def netG_forward(self, phase='test'):
        self.E = self.netG(self.A, self.B)  # 这里的self.E很可能是输出的融合结果
        # with torch.no_grad():
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward()  # 加载模型
        G_lossfn_type = self.opt_train['G_lossfn_type']  # vif
        ## constructe loss function  这里需要改动
        ...
        elif G_lossfn_type in ['mef', 'mff', 'vif', 'nir', 'med']:  # vif的loss计算方法
            total_loss, loss_text, loss_int, loss_contrast = self.G_lossfn(self.A, self.B, self.E)
        ...
        if G_lossfn_type in ['loe', 'mef', 'vif', 'mff', 'gt', 'nir', 'med']:
            ...
            self.log_dict['SSIM_loss'] = loss_contrast.item()
        ...
        # 2023年3月30日改，tensorboard的存储从image转为存储loss
        if G_lossfn_type == 'vif':
            if current_step % 100 == 0:
                self.writer.add_scalar('G_loss', G_loss.item(), current_step)
                self.writer.add_scalar('Text_loss',loss_text.item(), current_step)
                self.writer.add_scalar('Int_loss',loss_int.item(), current_step)
                self.writer.add_scalar('SSIM_loss', loss_contrast.item(),current_step)
