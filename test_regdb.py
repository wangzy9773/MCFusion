...
import time
import sys
from models.network_mcfusion import MCFusion as net
...
def main():
    ...
    parser.add_argument('--iter_number', type=str, default='20000')

    parser.add_argument('--root_path', type=str, default='./Dataset/testsets/',
                        help='input test image root folder')
    parser.add_argument('--dataset', type=str, default='RegDB', help='input test image name')
    parser.add_argument('--A_dir', type=str, default='Thermal', help='input test image name')
    parser.add_argument('--B_dir', type=str, default='Visible',help='input test image name')
    ...
    for i, test_data in enumerate(test_loader):
        ...
        save_path = os.path.join(save_dir,imgname.split('/')[-2])
        os.makedirs(save_path, exist_ok=True)
        save_name = os.path.join(save_path, os.path.basename(imgname).replace("_t_","_f_"))
...
def setup(args):
    save_dir = f'results/MCFusion_{str(ex_num)}_{args.iter_number}_{args.dataset}'
