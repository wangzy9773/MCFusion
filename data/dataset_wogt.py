...
class Dataset(data.Dataset): 
    ...
    def __getitem__(self, index):
        ...
        img_A = util.imread_uint(A_path, self.n_channels) #读取图像并变成 480*640*1
        img_B = util.imread_uint(B_path, self.n_channels)

        if self.opt['phase'] == 'train':   # 将图片处理成 128*128*1 且像素值除255
            """
            # --------------------------------
            ...
            # --------------------------------
            rnd_h = random.randint(0, max(0, H - self.patch_size)) #patch_size = 128
